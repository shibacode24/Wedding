      ...
      <div class="utf_box_widget margin-bottom-20" style="border-radius:20px; background-color:#fff; box-shadow: rgba(50, 50, 93, 0.25) 0px 6px 12px -2px, rgba(0, 0, 0, 0.3) 0px 3px 7px -3px;">
		  <h3>{{ $historys->hall_name->name ?? ''}}</h3>
		                        <!--<span style="font-size:18px; color:#000;"><b>Name of Wedding Venue</b></span><br>-->
                               	<span style="font-size:15px;"><b>Venue Type : {{ $historys->categoryname->category ?? ''}}</b></span><br>
                                <span style="font-size:15px;">Booking Date : {{date('d-m-Y',strtotime($historys->date))}}</span><br>
                                <span style="font-size:15px;">Booking Time : {{$historys->time_slot}}</span><br>
    <span style="font-size:15px;">Guest Capacity : {{$historys->guest}}</span><br>
		  @if($historys->status == '0')
		
		  <span style="font-size:15px;"><b>Status : <span style="color:rgb(243, 220, 18);">PENDING</span></b></span>
