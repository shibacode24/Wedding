...
// exit();
   ...
   public function view_enquiry()
   {
    ...
    ->leftjoin('listing_amenities','listing_amenities.id','=','booking.amenities_for_booking')
    ->select('booking.*','add_listing.user_id','listing_amenities.amenity','listing_amenities.price','listing_amenities.capacity')
    ->get();
...
if ($vendor) {
    ...
    if (isset($request->vendor_image) && !empty($request->vendor_image) && count($request->vendor_image) > 0) {
